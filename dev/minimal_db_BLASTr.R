...
teste20$`1_subject` %>% unique() %>% paste0(collapse = ",")
...
db_BLASTr <- dbConnect(SQLite(), "/home/heron/prjcts/BLASTr/dev/minimal_db/minimal_db_BLASTr.sql")

# get sequences for the minimal test database ----
  Seqs2DB(seqs  = "/home/heron/prjcts/BLASTr/dev/minimal_db/minimal_db_BLASTr.fasta",
          type =  "FASTA",dbFile =  db_BLASTr, identifier =  "BLASTr")

dna_BLASTr <- SearchDB(dbFile = db_BLASTr,identifier = "BLASTr",nameBy = "description")


# trimm too long sequences ----
dna_BLASTr[2] <- subseq(x = dna_BLASTr[2],
                        start = 885700,
                        end = 885900)
...
BLASTr_tbl <- teste20 %>%
  select(c("1_subject header", "1_subject","1_subject start", "1_subject end")) %>%
  mutate(START= if_else(`1_subject start` > `1_subject end`, `1_subject end`, `1_subject start`),
         END =  if_else(`1_subject start` > `1_subject end`, `1_subject start`, `1_subject end`)) %>%
...
dna_BLASTr[str_detect(string = names(dna_BLASTr),
...
for (seq in 1:nrow(BLASTr_tbl)) {
  ...
  dna_BLASTr[str_detect(string = names(dna_BLASTr),
                        pattern = BLASTr_tbl$`1_subject`[seq])] <-   subseq(x = dna_BLASTr[str_detect(string = names(dna_BLASTr),
                                                                                                      pattern = BLASTr_tbl$`1_subject`[seq])],
                                                                            start = if_else((BLASTr_tbl$START[seq] - 30 >=1),(BLASTr_tbl$START[seq] - 30),1),
         end = if_else((BLASTr_tbl$END[seq] + 30) >= BLASTr_tbl$END[seq],BLASTr_tbl$END[seq],(BLASTr_tbl$END[seq] + 30))
           )
...
Biostrings::writeXStringSet(x = dna_BLASTr,filepath = "/home/heron/prjcts/BLASTr/dev/minimal_db/shortest_minimal_db_BLASTr.fasta",format = "fasta")
...
teste_F2local_short <- furrr::future_map_dfr(ASVs_test,
                                             get_blast_results,
                                             num_thread = 2,
